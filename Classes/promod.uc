...
const VERSION_NAME = "promod_v1rc7";
...
var config int DefaultRoverHealth;
var config int PromodRoverHealth;
...
var config Array<class<Weapon> > WeaponInclusionList; //removed weapon list
var bool MTBalance; // Enable Mines, Turrets check
var bool BaseRape; // Enable BaseRape check
var bool SniperPlayerThreshold; // Enable Sniper Check
...
var config int SniperTeamPlayerMin; // Team Player Minimum per team required before the sniper gets enabled.
...
var bool DisplayStat; // Displays the promod ingame notification stats (Enabled/disabled baserape, etc)
...
var config bool CapperCanReinvo;
...
{
	local Engine.PlayerReplicationInfo P;

	forEach AllActors(class'Gameplay.PlayerReplicationInfo', P)
		if (P.bHasFlag)
		return true;
}
...
// Disable inventory when carrying a flag in tournament mode DOES NOT WANT TO COMPILE
ModifyInventoryStationAccess()
{
	local BaseDevice.InventoryStationAccess access;
	
	forEach AllActors(class'BaseDevice.InventoryStationAccess', access)
		{
//		if (PlayerReplicationInfo.HasFlag != None && TournamentOn())
		if (PlayerHasFlag() && TournamentOn())
			{
			access.basedeviceClass = class'promod_v1rc7.promodNoInvAccess';
			} else {
			access.basedeviceClass = class'promod_v1rc7.promodInvAccess';
			}
		}
	    ...
	    if(EnablePod)
	      {
			...
			vehiclePad.vehicleClass = class'promod_v1rc7.promodPod';
	      }
            else if(!EnablePod)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehiclePod')
            ...
            if(EnableRover)
	      {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleBuggy')
				...
				if(EnableRoverGun)
					{
					vehiclePad.vehicleClass = class'promod_v1rc7.promodDefaultBuggy';
					} else {
					vehiclePad.vehicleClass = class'promod_v1rc7.promodBuggy';
					}
				}
		      else if(!EnableRover)
				{
            ...
            if(EnableTank)
	      {
			...
			vehiclePad.vehicleClass = class'promod_v1rc7.promodTank';
	      }
            else if(!EnableTank)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleTank')
...
function ModifyCatapultINVHealth()
{
		...
		foreach AllActors(class'BaseObjectClasses.BaseInventoryStation', BaseInventoryStation)
			if(BaseInventoryStation != None)
				BaseInventoryStation.personalShieldClass = Class'BaseObjectClasses.ScreenMedium';
}
...
function ModifyStats()
{
	...
	if(M != None && BonusStatsOn && !TournamentOn() && CTFOn())
	{
		...
		// Head Shot
		M.projectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7.promodSniperProjectileDamageType';
...
//				 SniperRifle HS
//		M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7.promodSniperProjectileDamageType';
//		++statCount;
		
                // E-Blade
		M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7.promodBladeProjectileDamageType';
...
function Timer()
{
  ...
  local PlayerReplicationInfo P;
    ...
    if(MTBalance && !EnableMT() && DisplayStat)
	{
		MTBalance = false;
		UpdateMTDevices();
		Level.Game.BroadcastLocalized(self, class'promodGameMessage', 100);
	}
		else if(!MTBalance && EnableMT() && DisplayStat)
	{
		MTBalance = true;
		UpdateMTDevices();
		Level.Game.BroadcastLocalized(self, class'promodGameMessage', 101);
	}
	if(BaseRape && !EnableBR() && DisplayStat)
	{
		BaseRape = false;
		UpdateBRDevices();
		Level.Game.BroadcastLocalized(self, class'promodGameMessage', 103);
	}
        else if(!BaseRape && EnableBR() && DisplayStat)
	{
		BaseRape = true;
		UpdateBRDevices();
		Level.Game.BroadcastLocalized(self, class'promodGameMessage', 104);
	}
	if(SniperPlayerThreshold && !EnableSniper() && DisplayStat)
	{
		SniperPlayerThreshold = false;
		UpdateSniperRifleAllowance();
		Level.Game.BroadcastLocalized(self, class'promodGameMessage', 108);
	}
		else if(!SniperPlayerThreshold && EnableSniper() && DisplayStat)
	{
		SniperPlayerThreshold = true;
		UpdateSniperRifleAllowance();
		Level.Game.BroadcastLocalized(self, class'promodGameMessage', 109);
	}
    //Anti-TK
    for (C = Level.ControllerList; C != none; C = C.nextController)
    {
        if(PlayerController(C) == none && !MultiplayerGameInfo(Level.Game).bTournamentMode  && DisplayStat)
        continue;
...
function UpdateSniperRifleAllowance()
{
//	local Gameplay.Weapon weapon;
//	local Gameplay.Character character;	
	local Weapon weapon;
	local Character character;
	
	ForEach AllActors(Class'Character', character)
		if(character != None)
			if(ShouldModifyWeaponDevice(character))
				if(SniperPlayerThreshold)
					character.removeFromWeaponSlot(weapon);
}
...
function bool ShouldModifyWeaponDevice(Character character)
{
	local int i;

	for(i = 0; i < WeaponInclusionList.Length; i++)
		if(character.IsA(WeaponInclusionList[i].Name))
			return true;
        return false;
}
...
//		WeaponSniperRifle(Other).damageTypeClass = Class'promodSniperProjectileDamageType';
//		WeaponSniperRifle(Other).playerDamageStatClass = Class'Gameplay.ExtendedStat'
		return Super.ReplaceActor(Other);
	}
...
//		if(EnableRoverGun)
//			{
//			Buggy(Other).Health = DefaultRoverHealth;
//			} else {
//			Buggy(Other).Health = PromodRoverHealth;
//			}			
//			return Super.ReplaceActor(Other);
//	}
...
//		if(!TournamentOn())
//			{
			c.team().combatRoleData[0].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7.promodArmorLight", class'class')).default.AllowedWeapons;
			c.team().combatRoleData[1].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7.promodArmorMedium", class'class')).default.AllowedWeapons;
			c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7.promodArmorHeavy", class'class')).default.AllowedWeapons;
//			} else {
//			c.team().combatRoleData[0].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7.DefaultArmorLight", class'class')).default.AllowedWeapons;
//			c.team().combatRoleData[1].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7.DefaultArmorMedium", class'class')).default.AllowedWeapons;
//			c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynaSmicLoadObject("promod_v1rc7.DefaultArmorHeavy", class'class')).default.AllowedWeapons;
...
EnableRover=True
EnableRoverGun=True
;PromodRoverHealth=1500
;DefaultRoverHealth=1000
